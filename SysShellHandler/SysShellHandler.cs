...
namespace SysShellHandler
{
    public partial class SysShellHandler : Form
    {
        ...
        private void buttonMakeUsermode_Click(object sender, EventArgs e)
        {
            ...
            var ntoskrnl = Path.Combine(Path.GetDirectoryName(makeUsermode), "ntoskrnl.exe");
            if (File.Exists(ntoskrnl))
                File.Copy(ntoskrnl, Path.Combine(Path.GetDirectoryName(_driverFile), "ntoskrnl.exe"), true);
